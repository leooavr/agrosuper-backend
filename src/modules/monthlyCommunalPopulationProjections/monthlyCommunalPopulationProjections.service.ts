...
export class MonthlyCommunalPopulationProjectionsService {
    private logger: Logger = new Logger(MonthlyCommunalPopulationProjectionsService.name);

    constructor(
        private monthlyCommunalPopulationProjectionsRepository: MonthlyCommunalPopulationProjectionsRepository,
    ) {}

    async getMonthlyCommunalPopulationProjection(): Promise<MonthlyCommunalPopulationProjections[]> {
        try {
